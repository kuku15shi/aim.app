...
<head>
    ...
    <style>
        /* KSRTC Page Specific Styles - Red Theme */
        :root {
            --ksrtc-red: #d32f2f;
            --ksrtc-accent: #ff5252;
            --ksrtc-light: #ffebee;
        ...
        .bus-pulse-dot {
            width: 10px;
            height: 10px;
            background-color: var(--ksrtc-accent);
            border-radius: 50%;
            margin-right: 15px;
        ...
        .btn-find {
            ...
            margin-top: 20px;
            border-radius: 12px;
            background: linear-gradient(to right, #d32f2f 0%, #ff5252 100%);
            border: none;
            color: #fff;
            font-weight: 600;
            font-size: 1.1rem;
        ...
        .bus-card {
            ...
            padding: 20px;
            margin-bottom: 15px;
            border-left: 5px solid var(--ksrtc-red);
            display: flex;
            justify-content: space-between;
            align-items: center;
...
<div class="app-container">
        ...
        <div class="top-header">
            <div class="logo-section">
                ...
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <div class="main-text text-danger">KSRTC</div>
                        <div class="sub-text">RURAL & INTERCITY</div>
                    </div>
                </div>
            </div>
    ...
    <div class="bottom-nav">
        ...
        <a href="#" class="nav-item active">
            <div class="emergency-circle" style="background: linear-gradient(135deg, #d32f2f, #ff5252);">
...
function findBus() {
    ...
    resultsDiv.innerHTML = `
        <h5 class="mb-3 text-muted">Searching KSRTC Routes...</h5>
    ...
    // Fetch API with Type=ksrtc
    fetch(`api_bus.php?lat=${searchLat}&lng=${searchLng}&dest_id=${destId}&type=ksrtc`)
    ...
    .then(data => {
        ...
        let html = `
            <div class="mb-4 p-3 rounded-3" style="background: rgba(211, 47, 47, 0.1); border: 1px solid rgba(211, 47, 47, 0.2);">
                <small class="text-muted d-block uppercase" style="font-size: 0.75rem; letter-spacing: 1px;">NEAREST STOP FROM START</small>
                <h3 class="my-2" style="font-weight: 700; color: #d32f2f;">${data.nearest_stop.name}</h3>
                <a href="https://www.google.com/maps?q=${data.nearest_stop.lat},${data.nearest_stop.lng}" target="_blank" class="btn btn-sm btn-outline-danger rounded-pill">
                    <i class="fas fa-location-arrow me-1"></i> Navigate to Location
                </a>
            </div>
            <h5 class="mb-3 text-muted">Available KSRTC Buses</h5>
        `;

        if(data.routes.length === 0) {
            html += `<div class="text-center py-4 text-muted">
                        <i class="fas fa-bus fa-3x mb-3 opacity-50"></i>
                        <p>No KSRTC buses found connecting these locations.</p>
                     </div>`;
        } else {
                ...
                html += `
                    <div class="bus-card">
                        <div>
                            ...
                            <div class="bus-route badge bg-danger">${route.route_name}</div>
                            <div class="small text-muted mt-1">Trip: ${route.trip_name}</div>
                        </div>
                        <div class="text-end">
                            <div class="text-danger fw-bold mb-1"><i class="fas fa-hourglass-half me-1"></i> ${route.duration}</div>
